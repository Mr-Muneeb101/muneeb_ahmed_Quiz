...
// console.log([...data.results[2].incorrect_answers,data.results[2].correct_answer]);
interface WrongQuestions { question: string, inncorrect_ansewers: string[], correct_answer: string }
async function StartQuiz() {
    console.log(chalk.yellow.bold(`\n\n\t\t\t Welcome TO Muneeeb Ahmed Quiz Game\n\n`));

    let WrongQuestions: WrongQuestions[] = [];
    ...
    for (let i = 0; i < 5; i++) {
        ...
        let incorrect_answers: string[] = [...data.results[i].incorrect_answers];
        console.log(correct_answer);

        let answers: string[] = [...incorrect_answers];
        answers.splice(randomindexnumber, 0, correct_answer)
        let Question: string = data.results[i].question;
